...
exports.InterpreterRangeError = exports.InterpreterTypeError = exports.InterpreterReferenceError = exports.InterpreterSyntaxError = void 0;
exports.tokenize = tokenize;
exports.parse = parse;
exports.parseStatementList = parseStatementList;
exports.escapeString = escapeString;
exports.walkStackTrace = walkStackTrace;
exports.defaultPutProperty = defaultPutProperty;
exports.putProperty = putProperty;
exports.isPrimitive = isPrimitive;
exports.isObject = isObject;
exports.getDateObjectValue = getDateObjectValue;
exports.newObject = newObject;
exports.newNativeFunction = newNativeFunction;
...
function isExponentIndicator(char) {
    return char === "e" || char === "E";
}
function isIdentifierLetter(char) {
...
function formatUnexpectedCharacterError(syntax, expected, actual, position, context, caller, options) {
...
function formatUnexpectedTokenError(syntax, expected, actual, context, caller, options) {
    const stackTrace = context != null && caller != null ? walkStackTrace(context, caller) : [];
    stackTrace.unshift({
        name: "",
        start: actual.start,
...
function formatSyntaxError(syntax, message, token, context, caller, options) {
    const stackTrace = context != null && caller != null ? walkStackTrace(context, caller) : [];
    stackTrace.unshift({
        name: "",
        start: token.start,
...
class InterpreterSyntaxError extends Error {
    stackTrace;
    constructor(message, stackTrace, options) {
        super(`${message}\n${stackTrace.map(stackTraceToString).join("\n")}`, options);
        this.stackTrace = stackTrace;
    }
}
exports.InterpreterSyntaxError = InterpreterSyntaxError;
function stackTraceToString(s) {
...
exports.InterpreterReferenceError = InterpreterReferenceError;
...
exports.InterpreterTypeError = InterpreterTypeError;
...
exports.InterpreterRangeError = InterpreterRangeError;
class Reader {
    source;
    prevIndex;
    prevLine;
...
function* tokenize(source, sourceInfo) {
    ...
    while (!reader.eof) {
        ...
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("InputElement", "WhiteSpace or LineTerminator or Comment or Token", char, reader.position));
    }
    return { type: "end", value: "<EOF>", start: reader.position, end: reader.position };
}
function parseMultiLineComment(reader) {
    ...
    throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("MultiLineComment", "SourceCharacter", reader.current, reader.position));
}
function parseSingleLineComment(reader) {
    while (!reader.eof) {
        const char = reader.next();
        if (isLineTerminator(char)) {
            break;
...
function parseDecimalLiteral(reader, start) {
    ...
    if (reader.current === ".") {
        ...
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("DecimalLiteral", ". or DecimalDigit", reader.current, reader.position));
    }
...
function parseDecimalDigits(reader) {
    const char = reader.current;
    if (!isDecimalDigit(char)) {
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("DecimalDigits", "DecimalDigit", char, reader.position));
    }
    while (!reader.eof) {
        const char = reader.next();
        if (!isDecimalDigit(char)) {
...
function parseExponentPart(reader) {
    if (!isExponentIndicator(reader.current)) {
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("ExponentPart", "ExponentIndicator", reader.current, reader.position));
    }
...
function parseHexIntegerLiteral(reader, start) {
    const char = reader.next();
    if (!isHexDigit(char)) {
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("HexIntegerLiteral", "HexDigit", char, reader.position));
    }
    while (!reader.eof) {
...
function parseOctalIntegerLiteral(reader, start) {
    const char = reader.current;
    if (!isOctalDigit(char)) {
        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("OctalIntegerLiteral", "OctalDigit", char, reader.position));
    }
    while (!reader.eof) {
...
function parseReservedWordOrIdentifier(reader) {
    const start = reader.position;
    while (!reader.eof) {
        const char = reader.next();
    ...
    if (keywordsSet.has(value)) {
        return { type: "keyword", value: value, start, end: reader.prevPosition };
    }
    if (futureReservedWordsSet.has(value)) {
        return { type: "futureReservedWord", value: value, start, end: reader.prevPosition };
    }
    if (value === "null") {
...
function readStringLiteral(reader) {
            ...
            switch (char) {
                ...
                default:
                    if (char === "x") {
                        ...
                        if (!isHexDigit(u1)) {
                            throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("HexEscapeSequence", "HexDigit", reader.current, reader.position));
                        }
                        ...
                        const u1 = reader.next();
                        if (!isHexDigit(u1)) {
                            throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("UnicodeEscapeSequence", "HexDigit", reader.current, reader.position));
                        }
                        const u2 = reader.next();
                        if (!isHexDigit(u2)) {
                            throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("UnicodeEscapeSequence", "HexDigit", reader.current, reader.position));
                        ...
                        throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("StringLiteral", "not LineTerminator", reader.current, reader.position));
                    }
                    else {
                        value += char;
                    }
    ...
    throw new InterpreterSyntaxError(...formatUnexpectedCharacterError("StringLiteral", "SourceCharacter", reader.current, reader.position));
}
function readPunctuator(reader) {
    const start = reader.position;
...
function parse(source, sourceInfo) {
    const tokenizer = new Tokenizer(source, sourceInfo);
    return parseProgram(tokenizer);
}
function parseStatementList(source, state, sourceInfo) {
    const tokenizer = new Tokenizer(source, sourceInfo);
...
function parseSourceElement(tokenizer) {
...
function parseFormalParameterList(tokenizer) {
    const parameters = [];
    ...
    while (true) {
        ...
        if (parameter.type !== "identifier") {
            throw new InterpreterSyntaxError(...formatUnexpectedTokenError("FormalParameterList", "Identifier", parameter));
        }
        parameters.push(parameter.value);
    }
    return parameters;
}
function parseFunctionDeclaration(tokenizer) {
    const token = tokenizer.current;
    if (token.type !== "keyword" || token.value !== "function") {
...
function parseStatement(tokenizer, state) {
    const begin = tokenizer.current;
    ...
    throw new InterpreterSyntaxError(...formatUnexpectedTokenError("Statement", "{ or var or this or delete or void or typeof or new or Identifier or Literal or ( or ++ or -- or + or - or ~ or ! or ; or if or for or while or continue or break or return or with", begin));
}
function parseBlock(tokenizer, state) {
    const begin = tokenizer.current;
    if (begin.type !== "punctuator" || begin.value !== "{") {
...
function parseVariableStatement(tokenizer) {
    const begin = tokenizer.current;
    if (begin.type !== "keyword" || begin.value !== "var") {
    ...
    if (!parseSemicolon(tokenizer)) {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("VariableStatement", "; or } or LineTerminator", tokenizer.current));
    }
    return {
...
function parseVariableDeclarationList(tokenizer) {
    ...
    if (variableDeclarationList.length === 0) {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("VariableDeclarationList", "identifier", tokenizer.current));
    }
    return variableDeclarationList;
}
function parseVariableDeclaration(tokenizer) {
    const identifier = tokenizer.current;
    if (identifier.type !== "identifier") {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("VariableDeclaration", "Identifier", identifier));
    }
...
function parseSemicolon(tokenizer) {
    const token = tokenizer.current;
    if (token.type === "end") {
        return true;
    }
...
function parseExpressionStatement(tokenizer) {
    ...
    if (!parseSemicolon(tokenizer)) {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("ExpressionStatement", "; or } or LineTerminator", tokenizer.current));
    }
    return {
...
function parseIfStatement(tokenizer, state) {
    const begin = tokenizer.current;
    if (begin.type !== "keyword" || begin.value !== "if") {
...
function parseWhileStatement(tokenizer, state) {
    const begin = tokenizer.current;
    if (begin.type !== "keyword" || begin.value !== "while") {
...
function parseForStatement(tokenizer, state) {
    const begin = tokenizer.current;
    if (begin.type !== "keyword" || begin.value !== "for") {
    ...
    if (inOrSemicolon.type === "punctuator" && inOrSemicolon.value === ";") {
        ...
        let forInIntialization;
        if (initialization == null) {
            throw new InterpreterSyntaxError(...formatSyntaxError("ForStatement", "VariableDeclaration", tokenizer.current));
        }
        else if (initialization.type === "variableStatement") {
            if (initialization.variableDeclarationList[0] == null ||
            ...
            switch (initialization.type) {
                ...
                case "callOperator":
                    ...
                    break;
                default:
                    throw new InterpreterSyntaxError(...formatSyntaxError("ForStatement", "LeftHandSideExpression", tokenizer.current));
            }
        }
        tokenizer.next();
        const expression = parseExpression(tokenizer);
...
function parseContinueStatement(tokenizer, state) {
    const token = tokenizer.current;
    ...
    if (!state.while && !state.for) {
        throw new InterpreterSyntaxError(...formatSyntaxError("ContinueStatement", "continue statements are only allowed inside while/for", token));
    }
    tokenizer.next();
    if (!parseSemicolon(tokenizer)) {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("ContinueStatement", "; or } or LineTerminator", tokenizer.current));
    }
    return {
...
function parseBreakStatement(tokenizer, state) {
    const token = tokenizer.current;
    ...
    if (!state.while && !state.for) {
        throw new InterpreterSyntaxError(...formatSyntaxError("BreakStatement", "break statements are only allowed inside while/for", token));
    }
    tokenizer.next();
    if (!parseSemicolon(tokenizer)) {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("BreakStatement", "; or } or LineTerminator", tokenizer.current));
    }
    return {
        type: "breakStatement",
        start: token.start,
...
function parseReturnStatement(tokenizer, state) {
    const token = tokenizer.current;
    if (token.type !== "keyword" || token.value !== "return") {
        throw new InterpreterSyntaxError(...formatUnexpectedTokenError("ReturnStatement", "return", token));
    }
    if (!state.function) {
        throw new InterpreterSyntaxError(...formatSyntaxError("ReturnStatement", "return statements are only allowed inside functions", token));
    }
    tokenizer.next();
    if (!parseSemicolon(tokenizer)) {
        const expression = parseExpression(tokenizer);
        if (!parseSemicolon(tokenizer)) {
            throw new InterpreterSyntaxError(...formatUnexpectedTokenError("ReturnStatement", "; or } or LineTerminator", tokenizer.current));
        }
        return {
...
function parseWithStatement(tokenizer, state) {
    const begin = tokenizer.current;
    if (begin.type !== "keyword" || begin.value !== "with") {
...
function parsePrimaryExpression(tokenizer) {
    ...
    throw new InterpreterSyntaxError(...formatUnexpectedTokenError("PrimaryExpression", "this or Identifier or Literal or (", token));
}
function parseMemberExpression(tokenizer) {
    const begin = tokenizer.current;
    let left;
    ...
    while (true) {
        ...
        if (token.type === "punctuator" && token.value === ".") {
            ...
            if (token.type !== "identifier") {
                throw new InterpreterSyntaxError(...formatUnexpectedTokenError("MemberExpression", "Identifier", token));
            }
            left = {
                type: "memberOperator",
                left,
...
function parseLeftHandSideExpression(tokenizer) {
    let left = parseMemberExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "(") {
            left = {
...
function parseArgumentList(tokenizer) {
    const argumentList = [];
    const begin = tokenizer.current;
    if (begin.type !== "punctuator" || begin.value !== "(") {
...
function parsePostfixExpression(tokenizer) {
...
function parseUnaryExpression(tokenizer) {
    const begin = tokenizer.current;
    if (begin.type === "keyword") {
        switch (begin.value) {
            case "delete": {
...
function parseMultiplicativeExpression(tokenizer) {
    let left = parseUnaryExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "*") {
            tokenizer.next();
...
function parseShiftExpression(tokenizer) {
    let left = parseAdditiveExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "<<") {
            tokenizer.next();
...
function parseRelationalExpression(tokenizer) {
    let left = parseShiftExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "<") {
            tokenizer.next();
...
function parseEqualityExpression(tokenizer) {
    let left = parseRelationalExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "==") {
            tokenizer.next();
...
function parseBitwiseAndExpression(tokenizer) {
    let left = parseEqualityExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "&") {
            tokenizer.next();
...
function parseBitwiseXorExpression(tokenizer) {
    let left = parseBitwiseAndExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "^") {
            tokenizer.next();
...
function parseBitwiseOrExpression(tokenizer) {
    let left = parseBitwiseXorExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "|") {
            tokenizer.next();
...
function parseLogicalAndExpression(tokenizer) {
    let left = parseBitwiseOrExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "&&") {
            tokenizer.next();
...
function parseLogicalOrExpression(tokenizer) {
    let left = parseLogicalAndExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === "||") {
            tokenizer.next();
...
function parseConditionalExpression(tokenizer) {
    let conditionExpression = parseLogicalOrExpression(tokenizer);
    const token = tokenizer.current;
    if (token.type === "punctuator" && token.value === "?") {
        tokenizer.next();
        const thenExpression = parseAssignmentExpression(tokenizer);
        if (tokenizer.current.type !== "punctuator" || tokenizer.current.value !== ":") {
            throw new InterpreterSyntaxError(...formatUnexpectedTokenError("ConditionalExpression", ":", tokenizer.current));
        }
        tokenizer.next();
        return {
...
function parseAssignmentExpression(tokenizer) {
    const cond = parseConditionalExpression(tokenizer);
    if (cond.type === "thisExpression" ||
...
function parseExpression(tokenizer) {
    let left = parseAssignmentExpression(tokenizer);
    while (true) {
        const token = tokenizer.current;
        if (token.type === "punctuator" && token.value === ",") {
            tokenizer.next();
...
function walkStackTrace(context, caller) {
...
function canPutProperty(ctx, self, propertyName, caller) {
...
function defaultPutProperty(ctx, self, propertyName, value, caller) {
...
function* putProperty(ctx, self, propertyName, value, caller) {
...
function getNumberObjectValue(value) {
    if (isObject(value) &&
...
function getDateObjectValue(value) {
    if (isObject(value) &&
...
function toObject(intrinsics, value, ctx, caller) {
    switch (typeof value) {
        case "string":
...
function newStringObject(prototype, value) {
    return {
        internalProperties: {
            prototype: prototype,
            class: "String",
...
function newNumberObject(prototype, value) {
    return {
        internalProperties: {
            prototype,
            class: "Number",
...
function newBooleanObject(prototype, value) {
    return {
        internalProperties: {
            prototype,
            class: "Boolean",
...
function newNativeFunction(prototype, func, length, name) {
    return {
        internalProperties: {
            prototype,
            class: "Function",
...
function* getProperty(ctx, value, name, caller) {
    let o = value;
    while (o != null) {
...
function* callObject(ctx, obj, self, args, caller) {
...
function* defaultValue(ctx, value, hint, caller) {
    if (hint === "default") {
...
function* toPrimitive(ctx, value, preferredType, caller) {
    if (isPrimitive(value)) {
        return value;
    }
    return yield* defaultValue(ctx, value, preferredType, caller);
}
function* toString(ctx, value, caller) {
    if (value === undefined) {
        return "undefined";
    }
    if (value === null) {
...
function isValidStringNumericLiteral(value) {
    let i = 0;
...
function* toNumber(ctx, value, caller) {
    if (value === undefined) {
        return NaN;
    }
    if (value === null) {
...
function* toInt32(ctx, value, caller) {
...
function* constructFunction(ctx, args, caller) {
    let body = "";
    let argsStrings = [];
    ...
    let block;
    try {
        block = parseStatementList(body, { for: false, while: false, function: true }, { name: "anonymous", source: body });
    }
    catch (e) {
        if (e instanceof InterpreterSyntaxError) {
            throw new InterpreterSyntaxError(`Function: failed to parse function body: ${e}`, walkStackTrace(ctx, caller), { cause: e });
        }
        throw e;
    }
...
function* putArrayProperty(ctx, self, propertyName, value, caller) {
    if (!canPutProperty(ctx, self, propertyName, caller)) {
        return;
    }
...
function newArray(ctx, elements, length) {
    return {
        internalProperties: {
...
function* arrayConstructor(ctx, args, caller) {
    let len;
    let elements;
...
function* arrayPrototypeJoin(ctx, self, args, caller) {
    if (self == null) {
...
function* arrayPrototypeToString(ctx, self, _args, caller) {
    return yield* arrayPrototypeJoin(ctx, self, [], caller);
}
function* arrayPrototypeReverse(ctx, self, _args, caller) {
    if (self == null) {
...
function* mergeSort(ctx, comparefn, source, start, end, caller) {
...
function* arraySortCompare(ctx, x, y, caller, comparefn) {
...
function* arrayPrototypeSort(ctx, self, args, caller) {
    if (self == null) {
...
function* dateConstructor(ctx, args, caller) {
...
function* dateUTC(ctx, _self, args, caller) {
    args.length = Math.min(7, args.length);
    let numbers = [];
    for (const arg of args) {
        numbers.push(yield* toNumber(ctx, arg, caller));
    }
    return Date.UTC(...numbers);
}
function createIntrinsics() {
    ...
    functionPrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* functionToString(ctx, self, _args, caller) {
    ...
    objectPrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* objectToString(ctx, self, _args, caller) {
            // newer ES:
    ...
    objectPrototype.properties.set("valueOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* objectValueOf(_ctx, self, _args) {
            // FIXME: host object
            return self;
        }, 0, "valueOf"),
    });
    const string = newNativeFunction(functionPrototype, function* string(ctx, _, args, caller) {
        if (args.length === 0) {
            return "";
        }
        return yield* toString(ctx, args[0], caller);
    }, 1, "String");
    string.internalProperties.construct = function* stringConstructor(ctx, args, caller) {
    ...
    string.properties.set("fromCharCode", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringFromCharCode(ctx, _self, args, caller) {
            const codes = [];
            for (const arg of args) {
    ...
    stringPrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringToString(ctx, self, _args, caller) {
            if (!isObject(self) || typeof self.internalProperties.value !== "string") {
                throw new InterpreterTypeError("String.prototype.toString: this must be String object", ctx, caller);
            }
    ...
    stringPrototype.properties.set("valueOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringValueOf(ctx, self, _args, caller) {
            if (!isObject(self) || typeof self.internalProperties.value !== "string") {
    ...
    stringPrototype.properties.set("charAt", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringCharAt(ctx, self, args, caller) {
    ...
    stringPrototype.properties.set("charCodeAt", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringCharCodeAt(ctx, self, args, caller) {
            const str = yield* toString(ctx, self, caller);
            const pos = yield* toNumber(ctx, args[0], caller);
    ...
    stringPrototype.properties.set("indexOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringIndexOf(ctx, self, args, caller) {
            const str = yield* toString(ctx, self, caller);
    ...
    stringPrototype.properties.set("lastIndexOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringLastIndexOf(ctx, self, args, caller) {
            const str = yield* toString(ctx, self, caller);
            const searchStr = yield* toString(ctx, args[0], caller);
    ...
    stringPrototype.properties.set("split", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringSplit(ctx, self, args, caller) {
            const str = yield* toString(ctx, self, caller);
            if (args[0] === undefined) {
    ...
    stringPrototype.properties.set("substring", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringSubstring(ctx, self, args, caller) {
            const str = yield* toString(ctx, self, caller);
    ...
    stringPrototype.properties.set("toLowerCase", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringToLowerCase(ctx, self, _args, caller) {
            const str = yield* toString(ctx, self, caller);
    ...
    stringPrototype.properties.set("toUpperCase", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* stringToUpperCase(ctx, self, _args, caller) {
            const str = yield* toString(ctx, self, caller);
    ...
    const number = newNativeFunction(functionPrototype, function* Number(ctx, _, args, caller) {
        if (args.length === 0) {
            return 0;
        }
    ...
    numberPrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* numberToString(ctx, self, args, caller) {
            const value = getNumberObjectValue(self);
            if (value == null) {
                throw new InterpreterTypeError("Number.prototype.toString: this must be Number object", ctx, caller);
            }
    ...
    numberPrototype.properties.set("valueOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* numberValueOf(ctx, self, _args, caller) {
            const value = getNumberObjectValue(self);
            if (value == null) {
    ...
    const boolean = newNativeFunction(functionPrototype, function* Boolean(ctx, _, args) {
        if (args.length === 0) {
            return false;
        }
    ...
    booleanPrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* booleanToString(ctx, self, _args, caller) {
            if (!isObject(self) || typeof self.internalProperties.value !== "boolean") {
                throw new InterpreterTypeError("Boolean.prototype.toString: this must be Boolean object", ctx, caller);
            }
    ...
    booleanPrototype.properties.set("valueOf", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* booleanValueOf(ctx, self, _args, caller) {
            if (!isObject(self) || typeof self.internalProperties.value !== "boolean") {
                throw new InterpreterTypeError("Boolean.prototype.valueOf: this must be Boolean object", ctx, caller);
            }
            return self.internalProperties.value;
        }, 0, "valueOf"),
    });
    ...
    const array = newNativeFunction(functionPrototype, function* arrayConstruct(ctx, _self, args, caller) {
    ...
    const evalFunction = newNativeFunction(functionPrototype, 
    ...
    function* evalFunc(ctx, _self, args, caller) {
        ...
        try {
            ...
            if (e instanceof InterpreterSyntaxError) {
                throw new InterpreterSyntaxError(`eval: failed to parse program: ${e}`, walkStackTrace(ctx, caller), { cause: e });
            }
            throw e;
        }
    ...
    const parseIntFunction = newNativeFunction(functionPrototype, function* parseIntFunction(ctx, _self, args, caller) {
    ...
    const parseFloatFunction = newNativeFunction(functionPrototype, function* parseFloatFunction(ctx, _self, args, caller) {
        const string = yield* toString(ctx, args[0], caller);
        return parseFloat(string); // l
    }, 1, "parseFloat");
    const escapeFunction = newNativeFunction(functionPrototype, function* escapeFunction(ctx, _self, args, caller) {
        const string = yield* toString(ctx, args[0], caller);
        return escape(string); // l
    }, 1, "escape");
    const unescapeFunction = newNativeFunction(functionPrototype, function* unescapeFunction(ctx, _self, args, caller) {
        const string = yield* toString(ctx, args[0], caller);
        return unescape(string); // l
    }, 1, "unescape");
    const isNaNFunction = newNativeFunction(functionPrototype, function* isNaNFunction(ctx, _self, args, caller) {
        const number = yield* toNumber(ctx, args[0], caller);
        return isNaN(number);
    }, 1, "isNaN");
    const isFiniteFunction = newNativeFunction(functionPrototype, function* isFiniteFunction(ctx, _self, args, caller) {
        const number = yield* toNumber(ctx, args[0], caller);
        return isFinite(number);
    }, 1, "isFinite");
    const math = newObject(objectPrototype);
    math.internalProperties.class = "Math";
    for (const a of ["E", "LN10", "LN2", "LOG2E", "LOG10E", "PI", "SQRT1_2", "SQRT2"]) {
    ...
    for (const a of ["random"]) {
        math.properties.set(a, {
            ...
            dontEnum: true,
            dontDelete: false,
            value: newNativeFunction(functionPrototype, function* mathWrapper(_ctx, _self, _args) {
                return Math[a]();
            }, 0, a),
        });
    }
    for (const a of [
        ...
        math.properties.set(a, {
            ...
            dontEnum: true,
            dontDelete: false,
            value: newNativeFunction(functionPrototype, function* mathWrapper(ctx, _self, args, caller) {
                const a0 = yield* toNumber(ctx, args[0], caller);
                return Math[a](a0);
            }, 1, a),
        });
    }
    for (const a of ["atan2", "max", "min", "pow"]) {
        math.properties.set(a, {
            readOnly: false,
            dontEnum: true,
            dontDelete: false,
    ...
    const date = newNativeFunction(functionPrototype, function* date(_ctx, _self, _args) {
    ...
    date.properties.set("parse", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* dateParse(ctx, _self, args, caller) {
            const string = yield* toString(ctx, args[0], caller);
    ...
    datePrototype.properties.set("toString", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* dateToString(ctx, self, _args, caller) {
    ...
    for (const f of [
        ...
        datePrototype.properties.set(f, {
            ...
            dontEnum: true,
            dontDelete: false,
            value: newNativeFunction(functionPrototype, function* datePrototypeWrapper(ctx, self, _args, caller) {
                const value = getDateObjectValue(self);
                if (value == null) {
    ...
    for (const [f, length] of [
        ...
        datePrototype.properties.set(f, {
            ...
            dontEnum: true,
            dontDelete: false,
            value: newNativeFunction(functionPrototype, function* datePrototypeWrapper(ctx, self, args, caller) {
                const value = getDateObjectValue(self);
    ...
    datePrototype.properties.set("getYear", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* datePrototypeGetYear(ctx, self, _args, caller) {
            const value = getDateObjectValue(self);
            if (value == null) {
    ...
    datePrototype.properties.set("setYear", {
        ...
        dontEnum: true,
        dontDelete: false,
        value: newNativeFunction(functionPrototype, function* datePrototypeSetYear(ctx, self, args, caller) {
            const value = getDateObjectValue(self);
            if (value == null || !isObject(self)) {
...
function createGlobal(intrinsics) {
    return {
        internalProperties: {
            prototype: null,
            class: "Global",
...
function* referenceGetValue(ctx, reference, caller) {
...
function* referencePutValue(ctx, reference, value, caller) {
    if (!isReference(reference)) {
...
function* runEmptyStatement(_ctx, _statement) {
    return {
        type: "normalCompletion",
        hasValue: false,
    };
...
function* evaluateList(ctx, list, caller) {
    const result = [];
    for (const e of list) {
...
function compareString(intrinsics, primitiveLeft, primitiveRight) {
...
function* compareValue(ctx, leftValue, rightValue, caller) {
...
function* equalsValue(ctx, x, y, caller) {
...
function* multiply(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) * (yield* toNumber(ctx, right, caller));
}
function* divide(ctx, left, right, caller) {
    const r = (yield* toNumber(ctx, left, caller)) / (yield* toNumber(ctx, right, caller));
    return Number.isFinite(r) ? r | 0 : NaN;
}
function* modulo(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) % (yield* toNumber(ctx, right, caller));
}
function* add(ctx, left, right, caller) {
...
function* subtract(ctx, left, right, caller) {
    const r = (yield* toNumber(ctx, left, caller)) - (yield* toNumber(ctx, right, caller));
    return Number.isFinite(r) ? r | 0 : NaN;
}
function* leftShift(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) << (yield* toNumber(ctx, right, caller));
}
function* signedRightShift(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) >> (yield* toNumber(ctx, right, caller));
}
function* unsignedRightShift(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) >>> (yield* toNumber(ctx, right, caller));
}
function* bitwiseAnd(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) & (yield* toNumber(ctx, right, caller));
}
function* bitwiseOr(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) | (yield* toNumber(ctx, right, caller));
}
function* bitwiseXor(ctx, left, right, caller) {
    return (yield* toNumber(ctx, left, caller)) ^ (yield* toNumber(ctx, right, caller));
}
function* evaluateExpression(ctx, expression) {
    if (ctx.shouldInterrupt?.(expression)) {
        yield { type: "interruption", state: expression };
    }
    switch (expression.type) {
        ...
        case "memberOperator": {
            ...
            if ("name" in expression) {
                ...
                const right = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.right), expression);
                if (left == null) {
        ...
        case "newOperator": {
            const value = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.expression), expression);
            let args = [];
        ...
        case "logicalNotOperator":
            return !toBoolean(yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.expression), expression));
        case "multiplyOperator": {
            const left = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.left), expression.left);
            const right = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.right), expression.right);
        ...
        case "lessThanOperator": {
            const leftValue = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.left), expression.left);
            const rightValue = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.right), expression.right);
        ...
        case "conditionalOperator": {
            const condition = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.conditionExpression), expression.conditionExpression);
            if (toBoolean(condition)) {
                return yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.thenExpression), expression.thenExpression);
            }
            else {
                return yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, expression.elseExpression), expression.elseExpression);
            }
        }
...
function* runVariableDeclaration(ctx, declaration) {
...
function* runVariableStatement(ctx, statement) {
...
function* runExpressionStatement(ctx, statement) {
    const value = yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, statement.expression), statement.expression);
    return {
        type: "normalCompletion",
        hasValue: true,
        value,
...
function* runIfStatement(ctx, statement) {
...
function* runWhileStatement(ctx, statement) {
    let completion = {
        type: "normalCompletion",
        hasValue: false,
    };
...
function* runForStatement(ctx, statement) {
    let completion = {
        type: "normalCompletion",
        hasValue: false,
    };
    if (statement.initialization != null) {
        if (statement.initialization.type !== "variableStatement") {
            yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, statement.initialization), statement.initialization);
        }
        else {
    ...
    while (true) {
        if (statement.condition != null) {
            if (!toBoolean(yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, statement.condition), statement.condition))) {
                break;
            }
        }
        const result = yield* runStatement(ctx, statement.statement);
        ...
        if (statement.afterthought != null) {
            yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, statement.afterthought), statement.afterthought);
        }
    }
    return completion;
}
function* runForInStatement(ctx, statement) {
    ...
    let obj = toObject(ctx.realm.intrinsics, yield* referenceGetValue(ctx, yield* evaluateExpression(ctx, statement.expression), statement.expression), ctx, statement.expression);
    let completion = {
        type: "normalCompletion",
        hasValue: false,
    };
...
function* runReturnStatement(ctx, statement) {
...
function* runWithStatement(ctx, statement) {
    const ref = yield* evaluateExpression(ctx, statement.expression);
    const value = yield* referenceGetValue(ctx, ref, statement.expression);
...
function* runStatement(ctx, statement) {
    if (ctx.shouldInterrupt?.(statement)) {
        yield { type: "interruption", state: statement };
    }
    switch (statement.type) {
        case "block":
...
function findVariableObject(scope) {
...
function defineVariable(ctx, list) {
...
function newFunction(ctx, name, parameters, block) {
    function* call(ctx, self, args, caller) {
        const scope = {
        ...
        const iargToName = new Map();
        const nameToIArg = new Map();
        ...
        argumentsObject.internalProperties.put = function* (ctx, self, propertyName, value, caller) {
            const name = iargToName.get(propertyName);
            if (name != null) {
...
function findActivationObjectScope(scope) {
    let s = scope;
    while (s != null) {
        if (s.activation) {
            return s;
...
function* defineFunction(ctx, decl) {
    const func = newFunction(ctx, decl.name, decl.parameters, decl.block);
    yield* putProperty(ctx, findActivationObjectScope(ctx.scope)?.object ?? ctx.realm.globalObject, decl.name, func, decl);
}
...
async function runInContext(source, context, sourceInfo) {
...
function runAsync(source, sourceInfo) {
