...
exports.entityHeaderToString = entityHeaderToString;
exports.indexOf = indexOf;
exports.parseMediaTypeFromString = parseMediaTypeFromString;
...
//                | "/" | "[" | "]" | "?" | "="
//                | "{" | "}" | SP | HT
function isTSpecials(char) {
    return "()<>@,;:\\\"/[]?={} \t".indexOf(String.fromCharCode(char)) !== -1;
}
function entityHeaderToString(header) {
    return header.value.map(x => x.value).join("");
}
const utf8Encoder = new TextEncoder();
const utf8Decoder = new TextDecoder();
function indexOf(input, search, fromIndex) {
    if (search.length === 0) {
        return 0;
    }
...
function parseMediaTypeFromString(mediaType) {
...
class EntityParser {
    ...
    readMultipartEntityBody(mediaType) {
...
// > The type, subtype, and parameter attribute names are case-insensitive.  Parameter values may or may not be case-sensitive, depending on the semantics of the parameter name
// > Linear white space (LWS) MUST NOT be used between the type and subtype, nor between an attribute and its value.
function parseMediaType(tokens) {
    let offset = 0;
    ...
    function parseMediaTypeParameter() {
...
//# sourceMappingURL=entity_parser.js.map