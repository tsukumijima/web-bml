...
exports.bmlToXHTMLFXP = bmlToXHTMLFXP;
...
function renameXmlNode(node, name) {
...
function visitXmlNodes(node, callback) {
    callback(node);
    for (const child of getXmlChildren(node)) {
        visitXmlNodes(child, callback);
    }
}
function bmlToXHTMLFXP(data, cProfile) {
    const opts = {
    ...
    const parser = new fast_xml_parser_1.XMLParser(opts);
    ...
    const builder = new fast_xml_parser_1.XMLBuilder(opts);
    return builder.build(htmlChildren);
}
//# sourceMappingURL=bml_to_xhtml.js.map