...
exports.FontId = exports.BinaryWriter = void 0;
exports.toTTF = toTTF;
exports.loadDRCS = loadDRCS;
...
function map(ku, ten) {
    const s = jis_to_unicode_map_1.jisToUnicodeMap[(ku - 1) * 94 + ten - 1];
    if (typeof s === "number") {
        return s;
    }
    else {
...
function writeCMAP(glyphs, writer) {
...
function writeHEAD(_glyphs, writer) {
...
function writeHHEA(glyphs, writer) {
    writer.writeUInt16BE(1); // majorVersion
    writer.writeUInt16BE(0); // minorVersion
...
function writeHMTX(glyphs, writer) {
    for (let i = 0; i <= glyphs.length; i++) {
        writer.writeUInt16BE(1024);
        writer.writeInt16BE(0);
    }
}
function writeMAXP(glyphs, writer) {
...
function writeNAME(_glyphs, writer) {
...
function writeOS2(_glyphs, writer) {
...
function writePOST(_glyphs, writer) {
...
function writeCBLC(glyphs, writer) {
...
function findBestBitmap(glyphs, strike, glyphIndex) {
    ...
    return a[a.length - 1]?.bitmap;
}
function writeCBDT(glyphs, writer) {
    writer.writeUInt16BE(3);
    writer.writeUInt16BE(0);
    // SmallGlyphMetrics
    const strikes = getStrikes(glyphs);
...
function writeGLYF(glyphs, writer) {
    for (let i = 0; i <= glyphs.length; i++) {
...
function encodePNG({ width, height, bitmap, depth }) {
    ...
    view.setUint8(off, 0);
    off++;
    view.setInt32(off, crc_32_1.default.buf(buffer.subarray(ihdrOff, off)));
    off += 4;
    ...
    off += 4;
    view.setInt32(off, crc_32_1.default.buf(buffer.subarray(idatOff, off)));
    off += 4;
    ...
    view.setUint8(off, "D".charCodeAt(0));
    off++;
    view.setInt32(off, crc_32_1.default.buf(buffer.subarray(off - 4, off)));
    off += 4;
    return buffer;
}
let base64Table = [];
...
function writeSVG(glyphs, writer) {
    // SVGTableHeader
    writer.writeUInt16BE(0); // version
...
var FontId;
(function (FontId) {
    FontId[FontId["RoundGothic"] = 1] = "RoundGothic";
    FontId[FontId["SquareGothic"] = 2] = "SquareGothic";
    FontId[FontId["BoldRoundGothic"] = 3] = "BoldRoundGothic";
})(FontId || (exports.FontId = FontId = {}));
function loadDRCS(drcs, filterId) {
    let off = 0;
    ...
    for (let i = 0; i < nCode; i++) {
        ...
        off += 1;
        const glyphs = { ku: charCode1 - 0x20, ten: charCode2 - 0x20, glyphs: [] };
        for (let j = 0; j < nFont; j++) {
            ...
            if (mode === 0 || mode === 1) {
                ...
                if (filterId == null || fontId === filterId) {
