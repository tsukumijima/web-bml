...
exports.colorIndexToVar = colorIndexToVar;
exports.varToColorIndex = varToColorIndex;
...
const quirksProperties = new Set([
    "padding-top",
    "padding-right",
    "padding-bottom",
    "padding-left",
...
const colorIndexRules = new Map(Object.entries({
    "color-index": "color",
...
const bmlCssProperties = new Set([
    "nav-index",
    "nav-up",
    "nav-right",
    "nav-down",
...
function colorIndexToVar(colorIndex) {
    if (colorIndex == null) {
        return colorIndex;
    }
...
function varToColorIndex(colorIndexVar) {
...
async function processRule(node, opts) {
        ...
        if (rule.declarations) {
            if (rule.selectors != null) {
                ...
                rule.selectors = rule.selectors.map(selector => {
                    return selector.replace(/(?<=\s*):(focus|active)(?=\s*)/i, (_substring, pseudoClass) => {
...
async function transpileCSS(style, opts) {
    if (opts.inline) {
        style = "*{" + style + "}";
    }
    const parsed = css_1.default.parse(style);
    if (!parsed) {
        return style;
    }
    await processRule(parsed, opts);
    const transpiled = css_1.default.stringify(parsed, { compress: opts.inline });
    if (opts.inline) {
...
//# sourceMappingURL=transpile_css.js.map