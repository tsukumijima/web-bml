...
exports.BinaryTable = exports.BinaryTableType = exports.BinaryTableUnit = void 0;
exports.parseBinaryStructure = parseBinaryStructure;
exports.readBinaryFields = readBinaryFields;
exports.writeBinaryFields = writeBinaryFields;
const zip_code_1 = require("./zip_code");
var BinaryTableUnit;
(function (BinaryTableUnit) {
    BinaryTableUnit["Byte"] = "B";
    BinaryTableUnit["Bit"] = "b";
    BinaryTableUnit["Variable"] = "V";
})(BinaryTableUnit || (exports.BinaryTableUnit = BinaryTableUnit = {}));
var BinaryTableType;
(function (BinaryTableType) {
    BinaryTableType["Boolean"] = "B";
    BinaryTableType["UnsignedInteger"] = "U";
    BinaryTableType["Integer"] = "I";
    BinaryTableType["String"] = "S";
    BinaryTableType["ZipCode"] = "Z";
    BinaryTableType["Pad"] = "P";
})(BinaryTableType || (exports.BinaryTableType = BinaryTableType = {}));
var SearchOperator;
(function (SearchOperator) {
    SearchOperator[SearchOperator["Equal"] = 0] = "Equal";
    SearchOperator[SearchOperator["NotEqual"] = 1] = "NotEqual";
    SearchOperator[SearchOperator["Less"] = 2] = "Less";
    SearchOperator[SearchOperator["LessEqual"] = 3] = "LessEqual";
    SearchOperator[SearchOperator["Greater"] = 4] = "Greater";
    SearchOperator[SearchOperator["GreaterEqual"] = 5] = "GreaterEqual";
    SearchOperator[SearchOperator["And"] = 6] = "And";
    SearchOperator[SearchOperator["Or"] = 7] = "Or";
    SearchOperator[SearchOperator["Xor"] = 8] = "Xor";
    SearchOperator[SearchOperator["Nand"] = 9] = "Nand";
    SearchOperator[SearchOperator["Nor"] = 10] = "Nor";
    SearchOperator[SearchOperator["Nxor"] = 11] = "Nxor";
    SearchOperator[SearchOperator["StringMatches"] = 32] = "StringMatches";
    SearchOperator[SearchOperator["StringIncludes"] = 33] = "StringIncludes";
    SearchOperator[SearchOperator["StringStarsWith"] = 34] = "StringStarsWith";
    SearchOperator[SearchOperator["StringEndsWith"] = 35] = "StringEndsWith";
    SearchOperator[SearchOperator["StringNotMatch"] = 36] = "StringNotMatch";
    SearchOperator[SearchOperator["StringNotInclude"] = 37] = "StringNotInclude";
    SearchOperator[SearchOperator["BoolEqualTo"] = 64] = "BoolEqualTo";
    SearchOperator[SearchOperator["BoolNotEqualTo"] = 65] = "BoolNotEqualTo";
    SearchOperator[SearchOperator["ZipInclude"] = 96] = "ZipInclude";
    SearchOperator[SearchOperator["ZipNotInclude"] = 97] = "ZipNotInclude";
})(SearchOperator || (SearchOperator = {}));
function readBits(posBits, bits, buffer) {
    let value = 0;
...
function writeBits(posBits, bits, buffer, value) {
...
function parseBinaryStructure(structure) {
...
function readBinaryFields(buffer, fields, decodeText, posBits) {
    posBits = posBits ?? 0;
    const columns = [];
            ...
            case BinaryTableType.ZipCode:
                {
                    ...
                    fieldData = (0, zip_code_1.decodeZipCode)(zip);
                    posBits += lengthByte * 8;
                }
                break;
        }
...
function writeBinaryFields(data, fields, encodeText) {
    // Pad はデータ配列の要素ではないため、実データのフィールド数だけを検証する
...
class BinaryTable {
    rows;
    fields;
    constructor(table, structure, decodeText) {
        const { rows, fields } = BinaryTable.constructBinaryTable(table, structure, decodeText);
        this.rows = rows;
        this.fields = fields;
    }
    static constructBinaryTable(buffer, structure, decodeText) {
        const sep = structure.split(",");
        if (sep.length < 2) {
            throw new Error("FIXME");
        }
        ...
        while (posBits < buffer.length * 8) {
            ...
            // 固定長レコードも次のバイト境界から開始する (STD-B24 第二編付属2 5.5.2.4)
            posBits = lengthByte ? posBits + length * 8 : Math.ceil(read / 8) * 8;
            rows.push(columns);
        }
        // Pad は列を構成しないため、公開するフィールド一覧から除外する
    ...
    search(startRow, ...args) {
        // 検索条件は1～4組で、末尾の論理条件・件数・出力配列と合わせて3の倍数になる
        if (args.length % 3 !== 0 || args.length < 6 || args.length > 15) {
            return NaN;
        }
        const logic = args[args.length - 3];
        const limitCount = args[args.length - 2];
        ...
        // 行の走査前に列番号と演算子を検証し、不正な指定を NaN として処理する
        const conditions = [];
        for (let c = 0; c < args.length - 3; c += 3) {
            const searchedColumn = args[c];
            const compared = args[c + 1];
            const operator = args[c + 2];
            ...
            const isValidOperator = (field?.type === BinaryTableType.UnsignedInteger || field?.type === BinaryTableType.Integer) && operator >= SearchOperator.Equal && operator <= SearchOperator.Nxor ||
            ...
            const results = conditions.map(({ searchedColumn, compared, operator, field }) => {
                ...
                switch (operator) {
                    ...
                    case SearchOperator.ZipInclude:
                        result = (0, zip_code_1.zipCodeInclude)(column, Number(compared));
                        break;
            ...
            if (result) {
                resultArray.push(this.rows[i].map((v, j) => {
                    if (this.fields[j].type === BinaryTableType.ZipCode) {
                        // 同じ Zip 列に複数条件がある場合も、指定された AND/OR で結果をまとめる
...
//# sourceMappingURL=binary_table.js.map